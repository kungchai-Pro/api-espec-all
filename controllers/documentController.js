...
WHERE  sh.statusflow='105' AND sh.Actives='1' ORDER BY sh.stamptimeUpdate  DESC`;
    ...
    getJournalNotSuccessfully(req, res) {
        var dataselect = `SELECT ROW_NUMBER() OVER (ORDER BY sh.JournalID) AS num,sh.JournalID,sh.JournalCode,
sh.TransDate,sh.JournalGroupID,sh.ItemTypeID,sh.RefECN,sh.CustID,sh.ItemID,sh.ItemName,
sh.statusflow,
sh.stateflow,
(SELECT stateflow from espec_db.flowrunsystem fr WHERE fr.documentId=sh.JournalID order by fr.stateflow DESC LIMIT 1)as frstateflow,
sh.recievejob,
(CASE 
WHEN sh.statusflow='100' THEN 'Creat'
 WHEN sh.statusflow='101' THEN 'Draft'
...
(SELECT userall.name FROM espec_db.flowrunsystem 
inner join espec_db.userall on userall.userId=flowrunsystem.approveById
where documentId=sh.JournalID and stateflow=sh.stateflow limit 1) as toName,
(SELECT 
(SELECT u.name FROM userall u WHERE u.userId=createnoteby) as RejectBy
FROM transitionnote 
INNER JOIN SDSS_Head on SDSS_Head.JournalID=transitionnote.documentId 
	WHERE SDSS_Head.statusflow='201' AND SDSS_Head.JournalID=sh.JournalID ORDER BY transitionnote.noteId DESC LIMIT 1) as torejectName,
sh.UserIDConfirm,
(SELECT ac.name from espec_db.userall ac WHERE ac.userId=sh.UserIDConfirm) as createBy,
'111' as statusActions,
(SELECT fr.activerecieved from espec_db.flowrunsystem fr 
 WHERE fr.documentId=sh.JournalID AND fr.stateflow=sh.stateflow order by fr.stateflow DESC LIMIT 1) AS activerecieved,
 (SELECT fr.active from espec_db.flowrunsystem fr 
 WHERE fr.documentId=sh.JournalID AND fr.stateflow=sh.stateflow order by fr.stateflow DESC LIMIT 1) AS activeapproved,
sh.Actives,
sh.StandardCode,
sh.RefgroupCode,
...
WHERE   sh.Actives='1' and sh.statusflow!='105' AND sh.JournalGroupID='' ORDER BY sh.stamptimeUpdate  DESC`;
